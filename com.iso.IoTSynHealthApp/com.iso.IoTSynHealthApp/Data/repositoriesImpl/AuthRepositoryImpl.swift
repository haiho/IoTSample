...
final class AuthRepositoryImpl: AuthRepository {
    ...
    func login(email: String, password: String) async throws -> BaseAPIResponse<LoginResponse> {
        let request = LoginRequest(email: email, password: sha256(password))
        let result: BaseAPIResponse<LoginResponse> = try await APIService().request(
            path: "/signin",
            method: .POST,
            body: request,
    ...
    func register(
        ...
        let result: BaseAPIResponse< RegisterResponse> = try await APIService().request(
            path: "/signup",
            method: .POST,
            body: request,
